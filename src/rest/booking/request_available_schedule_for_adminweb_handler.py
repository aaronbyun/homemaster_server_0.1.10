...
class RequestAvailableSchedulesForAdminWebHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        for_manager                 = self.get_argument('for_manager', 1)
        ...
        try:
            ...
            available_schedules = scheduler.get_available_slots(gu_id = gu_id, geohash = geohash6, appointment_type = appointment_type, taking_time = total_taking_time_in_minutes,
                                                                ...
                                                                user_id = uid, by_manager = for_manager)
            ...
            # log to mixpanel
            mix.track(uid, 'request available schedule admin', {'time' : dt.datetime.now(), 'taking_time' : taking_time, 'additional_time' : additional_time, 'appointment_type' : appointment_type, 'have_pet' : have_pet, 'master_gender' : master_gender,'isdirty' : isdirty})

            # log to mongo
            mongo_logger.debug('request available schedule admin', extra = {'log_time' : dt.datetime.now(),  'user_id' : uid, 'taking_time' : taking_time, 'additional_time' : additional_time, 'appointment_type' : appointment_type, 'have_pet' : have_pet, 'master_gender' : master_gender,'isdirty' : isdirty})
