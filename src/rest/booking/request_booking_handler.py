...
class RequestBookingHandler(tornado.web.RequestHandler):
    def post(self):
            ...
            try:
                ...
                if ret_code:
                    ...
                    for bid in booking_ids:
                        print 'booking_id', booking_id, 'was added successfully...'

                    tid = msg
                    first_booking.tid    = tid
                    first_booking.status = BC.BOOKING_PAID
                    first_booking.clearning_status = BC.BOOKING_UPCOMMING
                    first_booking.payment_status = BC.BOOKING_PAID
                    ...
                    mongo_logger.debug('request booking failed to charge', extra = {    'uid' : uid, 'mid' : mid,
                                                                        'appointment_type' : appointment_type, 
                                                                        'have_pet' : have_pet, 'master_gender' : master_gender,
                                                                        'isdirty' : isdirty})
                    self.write(json.dumps(ret)) 
                ...
                mongo_logger.debug('request booking failed', extra = {    'uid' : uid, 'mid' : mid,
                                                                        'appointment_type' : appointment_type, 
                                                                        'have_pet' : have_pet, 'master_gender' : master_gender,
                                                                        'isdirty' : isdirty})
