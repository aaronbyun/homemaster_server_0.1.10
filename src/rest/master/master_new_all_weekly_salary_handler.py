...
class NewAllWeeklySalaryHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        try:
            ...
            for row in result:
                ...
                if prev_master_id == None or prev_master_id != row.Booking.master_id: # 변함
                    ...
                    salary['penalty_amount'] = 0 #masterdao.get_master_penalties(row.Booking.master_id, start_period, end_period)
            ...
            for ms in master_salaries:
                ...
                ms['actual_weekly_salary'] = '{:,}'.format(ms['weekly_salary'] - ms['penalty_amount'])

            total_salaries = '{:,}'.format(total_salaries)

            master_salaries = sorted(master_salaries, key = lambda x:(x['master_name']))

            ret['response'] = {'date_from' : last_saturday, 'date_to' : now, 'master_salaries' : master_salaries, 'total_salaries' : total_salaries}
